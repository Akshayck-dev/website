        ...
        if (distSq < minDist * minDist) {
          ...
          if (dist > 0.01) { // Avoid division by zero
            ...
            if (velocityAlongNormal > 0) {
              const m1 = p1.size; // Use mass proxy
              const m2 = p2.size;
...
export const ParticleMenuContent: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const menuItems = [
    { label: 'Home', href: '/' },
  ...
  return (
    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center pointer-events-none px-4">
      {/* Close button inside the menu itself (optional) */}
      <button 
        onClick={onClose}
        className="absolute top-8 right-8 text-white/70 hover:text-white pointer-events-auto transition-colors z-50 p-2"
      ...
      <div className="max-w-4xl w-full text-center space-y-10 pointer-events-auto">
        <div className="flex flex-col gap-6 items-center">
          {menuItems.map((item, index) => (
             <motion.a
               ...
               className="text-5xl md:text-7xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-white to-white/60 hover:from-blue-400 hover:to-blue-600 transition-all duration-300 tracking-tighter mix-blend-difference"
             >
               {item.label}
             </motion.a>
          ))}
        ...
        <div className="pt-12">
           <motion.div 
             ...
             className="flex items-center gap-2 justify-center text-white/30 text-sm"
           >
             <MousePointer2 size={16} />
             <span>Interact with the background</span>
           </motion.div>
        </div>
      </div>
    </div>
